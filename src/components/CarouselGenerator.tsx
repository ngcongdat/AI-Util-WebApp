...
export default function CarouselGenerator() {
  ...
  const handleGenerateSingleAI = async (index: number) => {
    ...
    setItems(items.map((it, i) => i === index ? { ...it, isGenerating: true } : it));
    try {
      const generated = await generateImageFromReference(baseImage, `Bối cảnh phù hợp với câu trích dẫn: "${items[index].quote}"`);
      setItems(prev => prev.map((it, i) => i === index ? { ...it, image: generated, isGenerating: false } : it));
    } catch {
      alert('Có lỗi xảy ra khi tạo ảnh bằng AI.');
      setItems(prev => prev.map((it, i) => i === index ? { ...it, isGenerating: false } : it));
    }
  };
  ...
  const handleGenerateRestWithAI = async (startIndex: number) => {
    ...
    try {
      for (let i = 0; i < items.length; i++) {
        ...
        setItems(prev => prev.map((it, idx) => idx === i ? { ...it, isGenerating: true } : it));
        setGenerationProgress(Math.round(((i + (i < startIndex ? 1 : 0)) / (items.length - 1)) * 100));
        const generated = await generateImageFromReference(baseImage, `Bối cảnh phù hợp với câu trích dẫn: "${items[i].quote}"`);
        setItems(prev => prev.map((it, idx) => idx === i ? { ...it, image: generated, isGenerating: false } : it));
      }
      setStep(3);
    } catch {
      alert('Có lỗi xảy ra khi tạo ảnh bằng AI.');
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-6 min-h-screen flex flex-col">
      ...
      <header className="mb-12 text-center">
        ...
        <p className="text-neutral-500 max-w-lg mx-auto">
          Tạo nội dung carousel chuyên nghiệp chỉ trong vài bước với sức mạnh của trí tuệ nhân tạo.
        </p>
      </header>
